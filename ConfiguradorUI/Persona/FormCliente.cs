...
namespace ConfiguradorUI.Persona
{
    public partial class FormCliente : MetroForm
    {
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Insertar)
                {
                    if (EsValido())
                    {
                        ...
                        obj = GetObjeto();
                        long id = new ClienteBL().InsertarCliente(obj);
            ...
            if (TipoOperacion == TipoOperacionABM.Eliminar)
            {
                if (dgvCliente.RowCount > 0)
                {
                    if (dgvCliente.SelectedRows.Count > 0)
                    {
                        ...
                        {
                            long id = 0;
                            if (long.TryParse(lblIdCliente.Text, out id) && id > 0)
                            {
                                ...
                                {
                                    new ClienteBL().EliminarCliente(id);
                                    ControlarEventosABM();
                                }
                            }
                            else
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Modificar && isSelected && isPending)
                {
                    if (EsValido())
                    {
                        ...
                        obj = GetObjeto();
                        long id = 0;
                        if (long.TryParse(lblIdCliente.Text, out id))
                        {
                            obj.id_cliente = id;
                            new ClienteBL().ActualizarCliente(obj);
                            ControlarEventosABM(obj.id_cliente);
                        }
                        isValid = true;
                    }
                    else { isValid = false; }
        ...
        private PERt02_cliente GetObjeto()
        {
            var obj = new PERt02_cliente();
            try
            {
                ...
                obj.txt_email1 = txtEmail01.Text.Trim().Trim();
                obj.txt_email2 = txtEmail02.Text.Trim().Trim();
        ...
        private void SetObjeto(PERt02_cliente obj)
        {
            try
            {
                ...
                lblIdCliente.Text = obj.id_cliente.ToString();
                codSelected = obj.cod_cliente;
        ...
        private bool EsValido()
        {
            ...
            if (no_error)
            {
                ...
                if (cod.Length > 0)
                {
                    ...
                    {
                        var obj = new ClienteBL().ClienteXCod(cod);
                        if (TipoOperacion == TipoOperacionABM.Insertar)
                        {
                            if (obj != null && obj.id_cliente > 0)
                            {
                                tabCliente.SelectedTab = tabPagGeneral;
                        ...
                        {
                            if (cod != codSelected && obj != null && obj.id_cliente > 0)
                            {
                                tabCliente.SelectedTab = tabPagGeneral;
                                MessageBox.Show("El código ya está en uso.", "MENSAJE EAGLE", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                errorProv.SetError(txtCodigo, "El código ya está en uso.");
            ...
            if (no_error == true)
            {
                ...
                foreach (var txtEm in txtsEmail)
                {
                    ...
                    if (email.Length > 0)
                    {
                        ...
                        {
                            tabCliente.SelectedTab = tabPagContacto;
        ...
        private void Filtrar(int criterio, string filtro)
        {
            ...
            try
            {
                ...
                {
                    DataGridViewRow row = dgvCliente.Rows
                    ...
                    if (row != null)
                    {
                        ...
                        {
                            dgvCliente.Rows[index].Selected = true;
                            dgvCliente.FirstDisplayedScrollingRowIndex = index;
                        }
                    }
                }
        ...
        private void SeleccionarPorId(long id)
        {
            ...
            try
            {
                ...
                DataGridViewRow row = dgvCliente.Rows
                .Cast<DataGridViewRow>()
                .Where(r => r.Cells["id_cliente"].Value.ToString().Equals(id.ToString()))
                .FirstOrDefault();
                if (row != null)
                {
                    index = row.Index;
        ...
        {
            isPending = false;
            if (dgvCliente.RowCount > 0 && dgvCliente.SelectedRows.Count > 0 && dgvCliente.CurrentRow.Index != -1)
            {
                long id = 0;
        ...
        private string GetIdSelected()
        {
            ...
            try
            {
                if (dgvCliente.SelectedRows.Count > 0 && dgvCliente.Rows.Count > 0)
                {
                    id = dgvCliente.SelectedRows[0].Cells[0].Value.ToString();
                }
            }
            catch (Exception e)
            {
        ...
        private void CargarGrilla(int? id_estado = null)
        {
            try
            {
                ...
                {
                    ContarEstados(lista);
                    dgvCliente.DataSource = listaView;
        ...
        {
            dgvCliente.RowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#ecf0f1");
            dgvCliente.AlternatingRowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#FAFAFA");

            //Cabecera
            dgvCliente.ColumnHeadersDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#00B2EE");
            dgvCliente.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            //Selección
            dgvCliente.DefaultCellStyle.SelectionBackColor = Color.DeepSkyBlue;
            ...
            dgvCliente.Columns["CODIGO"].Width = 100;
            dgvCliente.Columns["NOMBRE"].Width = 300;
        }
        ...
        private void ContarEstados(List<PERt02_cliente> lista)
        {
            try
            {
            ...
            if (isPending)
            {
                if (preguntar)
                {
                    ...
                    if (rp == DialogResult.Yes)
                    {
                        ...
                        if (idSelect != lblIdCliente.Text && idSelect != "-1")
                        {
                            isValid = Actualizar();
                            if (isValid)
                            {
