...
{
    public partial class FormCategoriaEmp : MetroForm
    {
        #region Variables
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Insertar)
                {
                    if (EsValido())
                    {
                        ...
                        obj = GetObjeto();
                        int id = new CategoriaEmpBL().InsertarCategoriaEmp(obj);
                        if (id > 0)
                            actualizar = true;
        ...
        private void Eliminar()
        {
            ...
            {
                if (dgvCategoriaEmp.RowCount > 0)
                {
                    if (dgvCategoriaEmp.SelectedRows.Count > 0)
                    {
                        try
                        {
                            int id = 0;
                            if (int.TryParse(lblIdCategoriaEmp.Text, out id) && id > 0)
                            {
                                ...
                                {
                                    bool validDelete = new UtilBL().ValidarDelete(id, CodValDelete.CategoriaEmp_Empleado);
                                    if (validDelete)
                                    {
                                        new CategoriaEmpBL().EliminarCategoriaEmp(id);
                                        actualizar = true;
                                        ControlarEventosABM();
                                    }
                                    else
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Modificar && isSelected && isPending)
                {
                    if (EsValido())
                    {
                        ...
                        obj = GetObjeto();
                        int id = 0;
                        if (int.TryParse(lblIdCategoriaEmp.Text, out id))
                        {
                            obj.id_categoria_emp = id;
                            new CategoriaEmpBL().ActualizarCategoriaEmp(obj);
                            actualizar = true;
                            ControlarEventosABM(obj.id_categoria_emp);
                        }
                        isValid = true;
                    }
                    else { isValid = false; }
        ...
        private PERt05_categoria_emp GetObjeto()
        {
            var obj = new PERt05_categoria_emp();
            try
            {
                ...
                obj.cod_categoria_emp = txtCodigo.Text.Trim();
        ...
        private void SetObjeto(PERt05_categoria_emp obj)
        {
            try
            {
                ...
                lblIdCategoriaEmp.Text = obj.id_categoria_emp.ToString();
                codSelected = obj.cod_categoria_emp;

                txtNombre.Text = obj.txt_nombre;
                txtDescripcion.Text = obj.txt_desc;
                txtCodigo.Text = obj.cod_categoria_emp;
        ...
        private bool EsValido()
        {
            ...
            if (no_error && !chkActivo.Checked && TipoOperacion == TipoOperacionABM.Modificar)
            {
                ...
                if (int.TryParse(lblIdCategoriaEmp.Text, out id))
                {
                    ...
                    validDelete = new UtilBL().ValidarDelete(id, CodValDelete.CategoriaEmp_Empleado);
                    if (!validDelete)
                    {
        ...
        private void Filtrar(int criterio, string filtro)
        {
            ...
            try
            {
                ...
                {
                    DataGridViewRow row = dgvCategoriaEmp.Rows
                    ...
                    {
                        index = row.Index;
                        if (dgvCategoriaEmp.Rows.Count > 0)
                        {
                            dgvCategoriaEmp.Rows[index].Selected = true;
                            dgvCategoriaEmp.FirstDisplayedScrollingRowIndex = index;
        ...
        private void SeleccionarPorId(int id)
        {
            ...
            try
            {
                ...
                DataGridViewRow row = dgvCategoriaEmp.Rows
                .Cast<DataGridViewRow>()
                .Where(r => r.Cells["id_categoria_emp"].Value.ToString().Equals(id.ToString()))
                .FirstOrDefault();
                if (row != null)
                {
                    index = row.Index;
        ...
        {
            isPending = false;
            if (dgvCategoriaEmp.RowCount > 0 && dgvCategoriaEmp.SelectedRows.Count > 0 && dgvCategoriaEmp.CurrentRow.Index != -1)
            {
                int id = 0;
                if (int.TryParse(GetIdSelected(), out id))
                {
                    if (id > 0)
                    {
                        var obj = new CategoriaEmpBL().CategoriaEmpXId(id);
                        if (obj != null)
                        {
                            isSelected = false;
                            SetObjeto(obj);
        ...
        private string GetIdSelected()
        {
            ...
            try
            {
                if (dgvCategoriaEmp.SelectedRows.Count > 0 && dgvCategoriaEmp.Rows.Count > 0)
                {
                    id = dgvCategoriaEmp.SelectedRows[0].Cells[0].Value.ToString();
                }
            }
            catch (Exception e)
            {
        ...
        private void CargarGrilla(int? id_estado = null)
        {
            try
            {
                var lista = new CategoriaEmpBL().ListaCategoriaEmp(id_estado, true);
                var listaView = lista.Select(x => new { x.id_categoria_emp, CODIGO = x.cod_categoria_emp, NOMBRE = x.txt_nombre })
               .OrderBy(x => string.IsNullOrEmpty(x.CODIGO)).ThenBy(x => x.CODIGO, new AlphaNumericComparer()).ThenBy(x => x.NOMBRE).ToList();

                if (lista != null)
                {
                    ContarEstados(lista);
                    dgvCategoriaEmp.DataSource = listaView;
                    dgvCategoriaEmp.Columns["id_categoria_emp"].Visible = false;
                }
            }
            catch (Exception e)
            {
        ...
        {
            dgvCategoriaEmp.RowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#ecf0f1");
            dgvCategoriaEmp.AlternatingRowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#FAFAFA");

            //Cabecera
            dgvCategoriaEmp.ColumnHeadersDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#00B2EE");
            dgvCategoriaEmp.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            //Selección
            dgvCategoriaEmp.DefaultCellStyle.SelectionBackColor = Color.DeepSkyBlue;
        ...
        private void dgvCategoriaEmp_SelectionChanged(object sender, EventArgs e)
        {
            errorProv.Clear();
            if (isPending)
            {
                if (preguntar)
                {
                    ...
                    if (rp == DialogResult.Yes)
                    {
                        ...
                        if (idSelect != lblIdCategoriaEmp.Text && idSelect != "-1")
                        {
                            isValid = Actualizar();
                            if (isValid)
                            {
