...
{
    public partial class FormTipoLocation : MetroForm
    {
        #region Variables
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Insertar)
                {
                    if (esValido())
                    {
                        ...
                        obj = GetObjeto();
                        int id = new TipoLocationBL().InsertarTipoLocation(obj);
                        if (id > 0)
                            actualizar = true;
        ...
        private void Eliminar()
        {
            ...
            {
                if (dgvTipoLocation.RowCount > 0)
                {
                    if (dgvTipoLocation.SelectedRows.Count > 0)
                    {
                        try
                        {
                            int id = 0;
                            if (int.TryParse(lblIdTipoLocation.Text, out id) && id > 0)
                            {
                                ...
                                {
                                    bool validDelete = new UtilBL().ValidarDelete(id, CodValDelete.TipoLocation_Location);
                                    if (validDelete)
                                    {
                                        new TipoLocationBL().EliminarTipoLocation(id);
                                        actualizar = true;
                                        ControlarEventosABM();
                                    }
                                    else
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Modificar && isSelected && isPending)
                {
                    if (esValido())
                    {
                        ...
                        obj = GetObjeto();
                        int id = 0;
                        if (int.TryParse(lblIdTipoLocation.Text, out id))
                        {
                            obj.id_tipo_location = id;
                            new TipoLocationBL().ActualizarTipoLocation(obj);
                            actualizar = true;
                            ControlarEventosABM(obj.id_tipo_location);
                        }
                        isValid = true;
                    }
                    else
        ...
        private MSTt09_tipo_location GetObjeto()
        {
            var obj = new MSTt09_tipo_location();
            try
            {
                ...
                obj.cod_tipo_location = txtCodigo.Text.Trim();
        ...
        private void SetObjeto(MSTt09_tipo_location obj)
        {
            try
            {
                ...
                lblIdTipoLocation.Text = obj.id_tipo_location.ToString();
                codSelected = obj.cod_tipo_location;
                txtNombre.Text = obj.txt_desc;
                txtAbreviacion.Text = obj.txt_abrv;
                txtCodigo.Text = obj.cod_tipo_location;
        ...
        private bool esValido()
        {
            ...
            {
                tabTipoLocation.SelectedTab = tabPagGeneral;
            ...
            if (no_error)
            {
                ...
                if (cod.Length > 0)
                {
                    ...
                    {
                        var obj = new TipoLocationBL().TipoLocationXCod(cod);
                        if (TipoOperacion == TipoOperacionABM.Insertar)
                        {
                            if (obj != null && obj.id_tipo_location > 0)
                            {
                                tabTipoLocation.SelectedTab = tabPagGeneral;
                        ...
                        {
                            if (cod != codSelected && obj != null && obj.id_tipo_location > 0)
                            {
                                tabTipoLocation.SelectedTab = tabPagGeneral;
                                MessageBox.Show("El código ya está en uso.", "MENSAJE EAGLE", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                errorProv.SetError(txtCodigo, "El código ya está en uso.");
            ...
            if (no_error && !chkActivo.Checked && TipoOperacion == TipoOperacionABM.Modificar)
            {
                ...
                if (int.TryParse(lblIdTipoLocation.Text, out id))
                {
                    ...
                    validDelete = new UtilBL().ValidarDelete(id, CodValDelete.TipoLocation_Location);
                    if (!validDelete)
                    {
        ...
        private void Filtrar(int criterio, string filtro)
        {
            ...
            try
            {
                ...
                {
                    DataGridViewRow row = dgvTipoLocation.Rows
                    ...
                    {
                        index = row.Index;
                        if (dgvTipoLocation.Rows.Count > 0)
                        {
                            dgvTipoLocation.Rows[index].Selected = true;
                            dgvTipoLocation.FirstDisplayedScrollingRowIndex = index;
                        }
                    }
                }
        ...
        private void SeleccionarPorId(int id)
        {
            ...
            try
            {
                ...
                DataGridViewRow row = dgvTipoLocation.Rows
                .Cast<DataGridViewRow>()
                .Where(r => r.Cells["id_tipo_location"].Value.ToString().Equals(id.ToString()))
                .FirstOrDefault();
                if (row != null)
                {
                    index = row.Index;
        ...
        {
            isPending = false;
            if (dgvTipoLocation.RowCount > 0 && dgvTipoLocation.SelectedRows.Count > 0 && dgvTipoLocation.CurrentRow.Index != -1)
            {
                int id = 0;
                if (int.TryParse(GetIdSelected(), out id))
                {
                    if (id > 0)
                    {
                        var obj = new TipoLocationBL().TipoLocationXId(id);
                        if (obj != null)
                        {
                            isSelected = false;
                            SetObjeto(obj);
        ...
        private string GetIdSelected()
        {
            ...
            try
            {
                if (dgvTipoLocation.SelectedRows.Count > 0 && dgvTipoLocation.Rows.Count > 0)
                {
                    id = dgvTipoLocation.SelectedRows[0].Cells[0].Value.ToString();
                }
            }
            catch (Exception e)
            {
        ...
        private void CargarGrilla(int? id_estado = null)
        {
            try
            {
                var lista = new TipoLocationBL().ListaTipoLocation(id_estado, true);
                var listaView = lista.Select(x => new { x.id_tipo_location, CODIGO = x.cod_tipo_location, NOMBRE = x.txt_desc })
               .OrderBy(x => string.IsNullOrEmpty(x.CODIGO)).ThenBy(x => x.CODIGO, new AlphaNumericComparer()).ThenBy(x => x.NOMBRE).ToList();

                if (lista != null)
                {
                    ContarEstados(lista);
                    dgvTipoLocation.DataSource = listaView;
                    dgvTipoLocation.Columns["id_tipo_location"].Visible = false;
                }
            }
            catch (Exception e)
            {
        ...
        {
            dgvTipoLocation.RowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#ecf0f1");
            dgvTipoLocation.AlternatingRowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#FAFAFA");

            //Cabecera
            dgvTipoLocation.ColumnHeadersDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#00B2EE");
            dgvTipoLocation.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            //Selección
            dgvTipoLocation.DefaultCellStyle.SelectionBackColor = Color.DeepSkyBlue;

            //Para que no sobreescriba los estilos de cabecera
            dgvTipoLocation.EnableHeadersVisualStyles = false;

            //Configurando columnas del grid
            dgvTipoLocation.AllowUserToResizeColumns = true;
            dgvTipoLocation.Columns["CODIGO"].HeaderText = "CÓDIGO";

            dgvTipoLocation.Columns["CODIGO"].Width = 100;
            dgvTipoLocation.Columns["NOMBRE"].Width = 300;
        }
        ...
        private void ContarEstados(List<MSTt09_tipo_location> lista)
        {
            try
            {
        ...
        private void FormTipoLocation_Load(object sender, EventArgs e)
        {
            lblIdTipoLocation.Visible = false;
            SetMaxLengthTxt();
            ControlarEventosABM();
            LimpiarForm();
        ...
        private void dgvTipoLocation_SelectionChanged(object sender, EventArgs e)
        {
            errorProv.Clear();
            if (isPending)
            {
                if (preguntar)
                {
                    ...
                    if (rp == DialogResult.Yes)
                    {
                        ...
                        if (idSelect != lblIdTipoLocation.Text && idSelect != "-1")
                        {
                            isValid = Actualizar();
                            if (isValid)
                            {
