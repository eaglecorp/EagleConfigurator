...
{
    public partial class FormMedioPago : MetroForm
    {
        #region Variables
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Insertar)
                {
                    if (EsValido())
                    {
                        ...
                        obj = GetObjeto();
                        int id = new MedioPagoBL().InsertarMedioPago(obj);
        ...
        private void Eliminar()
        {
            if (TipoOperacion == TipoOperacionABM.Eliminar)
            {
                ...
                {
                    if (dgvMedioPago.SelectedRows.Count > 0)
                    {
                        try
                        {
                            int id = 0;
                            if (int.TryParse(lblIdMedioPago.Text, out id) && id > 0)
                            {
                                ...
                                {
                                    new MedioPagoBL().EliminarMedioPago(id);
                                    ControlarEventosABM();
                                }
                            }
                            else
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Modificar && isSelected && isPending)
                {
                    if (EsValido())
                    {
                        ...
                        obj = GetObjeto();
                        int id = 0;
                        if (int.TryParse(lblIdMedioPago.Text, out id))
                        {
                            obj.id_medio_pago = id;
                            new MedioPagoBL().ActualizarMedioPago(obj);
                            ControlarEventosABM(obj.id_medio_pago);
                        }
                        isValid = true;
                    }
                    else { isValid = false; }
        ...
        private MSTt01_medio_pago GetObjeto()
        {
            var obj = new MSTt01_medio_pago();
            try
            {
                ...
                if (cboTipoMedioPago.SelectedValue != null)
                    obj.id_tipo_medio_pago = int.Parse(cboTipoMedioPago.SelectedValue.ToString());
                else obj.id_tipo_medio_pago = null;
            }
            catch (Exception e)
            {
        ...
        private void SetObjeto(MSTt01_medio_pago obj)
        {
            try
            {
                ...
                txtCodigo.Text = obj.cod_medio_pago;

                if (obj.id_tipo_medio_pago != null)
                    cboTipoMedioPago.SelectedValue = obj.id_tipo_medio_pago;
                else
                    cboTipoMedioPago.SelectedIndex = -1;
        ...
        private void Filtrar(int criterio, string filtro)
        {
            ...
            try
            {
                ...
                {
                    DataGridViewRow row = dgvMedioPago.Rows
                    ...
                    {
                        index = row.Index;
                        if (dgvMedioPago.Rows.Count > 0)
                        {
                            dgvMedioPago.Rows[index].Selected = true;
                            dgvMedioPago.FirstDisplayedScrollingRowIndex = index;
                        }
                    }
                }
        ...
        private void SeleccionarPorId(int id)
        {
            ...
            try
            {
                ...
                DataGridViewRow row = dgvMedioPago.Rows
                .Cast<DataGridViewRow>()
                .Where(r => r.Cells["id_medio_pago"].Value.ToString().Equals(id.ToString()))
                .FirstOrDefault();
                if (row != null)
                {
                    index = row.Index;
        ...
        {
            isPending = false;
            if (dgvMedioPago.RowCount > 0 && dgvMedioPago.SelectedRows.Count > 0 && dgvMedioPago.CurrentRow.Index != -1)
            {
                int id = 0;
                if (int.TryParse(GetIdSelected(), out id))
                {
                    if (id > 0)
                    {
                        var obj = new MedioPagoBL().MedioPagoXId(id);
                        if (obj != null)
                        {
                            isSelected = false;
                            SetObjeto(obj);
        ...
        private string GetIdSelected()
        {
            ...
            try
            {
                if (dgvMedioPago.SelectedRows.Count > 0 && dgvMedioPago.Rows.Count > 0)
                {
                    id = dgvMedioPago.SelectedRows[0].Cells[0].Value.ToString();
                }
            }
            catch (Exception e)
            {
        ...
        private void CargarCombos()
        {
            try
            {
                cboTipoMedioPago.DataSource = null;
                cboTipoMedioPago.DisplayMember = "txt_desc";
                cboTipoMedioPago.ValueMember = "id_tipo_medio_pago";
                cboTipoMedioPago.DataSource = new TipoMedioPagoBL().ListaTipoMedioPago(Estado.IdActivo, true);
            }
            catch (Exception e)
            {
        ...
        private void CargarGrilla(int? id_estado = null)
        {
            try
            {
               ...
               .OrderBy(x => string.IsNullOrEmpty(x.CODIGO)).ThenBy(x => x.CODIGO, new AlphaNumericComparer()).ThenBy(x => x.NOMBRE).ToList();
                ...
                {
                    ContarEstados(lista);
                    dgvMedioPago.DataSource = listaView;
                    dgvMedioPago.Columns["id_medio_pago"].Visible = false;
                }
            }
            catch (Exception e)
            {
        ...
        {
            dgvMedioPago.RowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#ecf0f1");
            dgvMedioPago.AlternatingRowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#FAFAFA");

            //Cabecera
            dgvMedioPago.ColumnHeadersDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#00B2EE");
            dgvMedioPago.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            //Selección
            dgvMedioPago.DefaultCellStyle.SelectionBackColor = Color.DeepSkyBlue;

            //Para que no sobreescriba los estilos de cabecera
            dgvMedioPago.EnableHeadersVisualStyles = false;

            //Configurando columnas del grid
            dgvMedioPago.AllowUserToResizeColumns = true;
            dgvMedioPago.Columns["CODIGO"].HeaderText = "CÓDIGO";

            dgvMedioPago.Columns["CODIGO"].Width = 100;
            dgvMedioPago.Columns["NOMBRE"].Width = 300;
        }
        ...
        private void ContarEstados(List<MSTt01_medio_pago> lista)
        {
            try
            {
        ...
        private void dgvMedioPago_SelectionChanged(object sender, EventArgs e)
        {
            errorProv.Clear();
            if (isPending)
            {
                if (preguntar)
                {
                    ...
                    if (rp == DialogResult.Yes)
                    {
                        ...
                        if (idSelect != lblIdMedioPago.Text && idSelect != "-1")
                        {
                            isValid = Actualizar();
                            if (isValid)
                            {
