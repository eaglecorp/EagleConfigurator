...
namespace ConfiguradorUI.Labor
{
    public partial class FormTrabajo : MetroForm
    {
        ...
        private void Eliminar()
        {
            ...
            {
                if (dgvTrabajo.RowCount > 0)
                {
                    if (dgvTrabajo.SelectedRows.Count > 0)
                    {
                        try
                        {
                            int id = 0;
                            if (int.TryParse(lblIdTrabajo.Text, out id) && id > 0)
                            {
                                ...
                                {
                                    bool validDelete = new UtilBL().ValidarDelete(id, CodValDelete.Trabajo_Empleado);
                                    if (validDelete)
                                    {
                                        new TrabajoBL().EliminarTrabajo(id);
                                        actualizar = true;
                                        ControlarEventosABM();
                                    }
                                    else
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Modificar && isSelected && isPending)
                {
                    if (EsValido())
                    {
                        ...
                        {
                            obj.id_trabajo = id;
                            new TrabajoBL().ActualizarTrabajo(obj);
                            actualizar = true;
                            ControlarEventosABM(obj.id_trabajo);
                        }
                        isValid = true;
                    }
                    else { isValid = false; }
        ...
        private LABt06_trabajo GetObjeto()
        {
            ...
            try
            {
                ...
                obj.cod_trabajo = txtCodigo.Text.Trim();
        ...
        private void SetObjeto(LABt06_trabajo obj)
        {
            try
            {
                ...
                lblIdTrabajo.Text = obj.id_trabajo.ToString();
                codSelected = obj.cod_trabajo;

                txtNombre.Text = obj.txt_nombre;
                txtDescripcion.Text = obj.txt_desc;
                txtCodigo.Text = obj.cod_trabajo;
        ...
        private bool EsValido()
        {
            ...
            {
                tabTrabajo.SelectedTab = tabPagGeneral;
            ...
            if (no_error)
            {
                ...
                if (cod.Length > 0)
                {
                    ...
                    {
                        var obj = new TrabajoBL().TrabajoXCod(cod);
                        if (TipoOperacion == TipoOperacionABM.Insertar)
                        {
                            if (obj != null && obj.id_trabajo > 0)
                            {
                                tabTrabajo.SelectedTab = tabPagGeneral;
                        ...
                        {
                            if (cod != codSelected && obj != null && obj.id_trabajo > 0)
                            {
                                tabTrabajo.SelectedTab = tabPagGeneral;
                                MessageBox.Show("El código ya está en uso.", "MENSAJE EAGLE", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                errorProv.SetError(txtCodigo, "El código ya está en uso.");
            ...
            if (no_error && !chkActivo.Checked && TipoOperacion == TipoOperacionABM.Modificar)
            {
                ...
                if (int.TryParse(lblIdTrabajo.Text, out id))
                {
                    ...
                    validDelete = new UtilBL().ValidarDelete(id, CodValDelete.Trabajo_Empleado);
                    if (!validDelete)
                    {
        ...
        private void Filtrar(int criterio, string filtro)
        {
            ...
            try
            {
                ...
                {
                    DataGridViewRow row = dgvTrabajo.Rows
                    ...
                    {
                        index = row.Index;
                        if (dgvTrabajo.Rows.Count > 0)
                        {
                            dgvTrabajo.Rows[index].Selected = true;
                            dgvTrabajo.FirstDisplayedScrollingRowIndex = index;
        ...
        private void SeleccionarPorId(int id)
        {
            ...
            try
            {
                ...
                DataGridViewRow row = dgvTrabajo.Rows
                .Cast<DataGridViewRow>()
                .Where(r => r.Cells["id_trabajo"].Value.ToString().Equals(id.ToString()))
                .FirstOrDefault();
                if (row != null)
                {
                    index = row.Index;
        ...
        {
            isPending = false;
            if (dgvTrabajo.RowCount > 0 && dgvTrabajo.SelectedRows.Count > 0 && dgvTrabajo.CurrentRow.Index != -1)
            {
                int id = 0;
                if (int.TryParse(GetIdSelected(), out id))
                {
                    if (id > 0)
                    {
                        var obj = new TrabajoBL().TrabajoXId(id);
                        if (obj != null)
                        {
                            isSelected = false;
                            SetObjeto(obj);
        ...
        private string GetIdSelected()
        {
            ...
            try
            {
                if (dgvTrabajo.SelectedRows.Count > 0 && dgvTrabajo.Rows.Count > 0)
                {
                    id = dgvTrabajo.SelectedRows[0].Cells[0].Value.ToString();
                }
            }
            catch (Exception e)
            {
        ...
        private void CargarGrilla(int? id_estado = null)
        {
            try
            {
                var lista = new TrabajoBL().ListaTrabajo(id_estado);
                var listaView = lista.Select(x => new { x.id_trabajo, CODIGO = x.cod_trabajo, NOMBRE = x.txt_nombre })
               .OrderBy(x => string.IsNullOrEmpty(x.CODIGO)).ThenBy(x => x.CODIGO, new AlphaNumericComparer()).ThenBy(x => x.NOMBRE).ToList();

                if (lista != null)
                {
                    ContarEstados(lista);
                    dgvTrabajo.DataSource = listaView;
                    dgvTrabajo.Columns["id_trabajo"].Visible = false;
                }
            }
            catch (Exception e)
            {
        ...
        {
            dgvTrabajo.RowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#ecf0f1");
            dgvTrabajo.AlternatingRowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#FAFAFA");

            //Cabecera
            dgvTrabajo.ColumnHeadersDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#00B2EE");
            dgvTrabajo.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            //Selección
            dgvTrabajo.DefaultCellStyle.SelectionBackColor = Color.DeepSkyBlue;

            //Para que no sobreescriba los estilos de cabecera
            dgvTrabajo.EnableHeadersVisualStyles = false;

            //Configurando columnas del grid
            dgvTrabajo.AllowUserToResizeColumns = true;
            dgvTrabajo.Columns["CODIGO"].HeaderText = "CÓDIGO";

            dgvTrabajo.Columns["CODIGO"].Width = 100;
            dgvTrabajo.Columns["NOMBRE"].Width = 300;
        }
        ...
        private void dgvTrabajo_SelectionChanged(object sender, EventArgs e)
        {
            errorProv.Clear();
            if (isPending)
            {
                if (preguntar)
                {
                    ...
                    if (rp == DialogResult.Yes)
                    {
                        ...
                        if (idSelect != lblIdTrabajo.Text && idSelect != "-1")
                        {
                            isValid = Actualizar();
                            if (isValid)
                            {
