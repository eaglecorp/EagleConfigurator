...
{
    public partial class FormTipoProd : MetroForm
    {
        #region Variables
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Insertar)
                {
                    if (esValido())
                    {
                        ...
                        obj = GetObjeto();
                        int id = new TipoProdBL().InsertarTipoProd(obj);
                        if (id > 0)
                            actualizar = true;
            ...
            if (TipoOperacion == TipoOperacionABM.Eliminar)
            {
                if (dgvTipoProd.RowCount > 0)
                {
                    if (dgvTipoProd.SelectedRows.Count > 0)
                    {
                        ...
                        {
                            int id = 0;
                            if (int.TryParse(lblIdTipoProd.Text, out id) && id > 0)
                            {
                                ...
                                {
                                    bool validDelete = new UtilBL().ValidarDelete(id, CodValDelete.TipoProd_Producto);
                                    if (validDelete)
                                    {
                                        new TipoProdBL().EliminarTipoProd(id);
                                        actualizar = true;
                                        ControlarEventosABM();
                                    }
                                    else
        ...
        private bool Actualizar()
        {
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Modificar && isSelected && isPending)
                {
                    if (esValido())
                    {
                        ...
                        obj = GetObjeto();
                        int id = 0;
                        if (int.TryParse(lblIdTipoProd.Text, out id))
                        {
                            obj.id_tipo_prod = id;
                            new TipoProdBL().ActualizarTipoProd(obj);
                            actualizar = true;
                            ControlarEventosABM(obj.id_tipo_prod);
                        }
                        isValid = true;
                    }
                    else { isValid = false; }
            ...
            {
                MessageBox.Show("Ocurrió una excepción en Actualizar registro: " + e.Message);
            }
            return isValid;
        }
        ...
        private PROt07_tipo_prod GetObjeto()
        {
            var obj = new PROt07_tipo_prod();
            try
            {
                obj.txt_desc = txtNombre.Text.Trim();
                obj.cod_tipo_prod = txtCodigo.Text.Trim();
        ...
        private void SetObjeto(PROt07_tipo_prod obj)
        {
            try
            {
                ...
                lblIdTipoProd.Text = obj.id_tipo_prod.ToString();
                codSelected = obj.cod_tipo_prod;

                txtNombre.Text = obj.txt_desc;
                txtCodigo.Text = obj.cod_tipo_prod;
        ...
        private bool esValido()
        {
            ...
            {
                tabTipoProd.SelectedTab = tabPagGeneral;
            ...
            if (no_error)
            {
                ...
                if (cod.Length > 0)
                {
                    ...
                    {
                        var obj = new TipoProdBL().TipoProdXCod(cod);
                        if (TipoOperacion == TipoOperacionABM.Insertar)
                        {
                            if (obj != null && obj.id_tipo_prod > 0)
                            {
                                tabTipoProd.SelectedTab = tabPagGeneral;
                        ...
                        {
                            if (cod != codSelected && obj != null && obj.id_tipo_prod > 0)
                            {
                                tabTipoProd.SelectedTab = tabPagGeneral;
                                MessageBox.Show("El código ya está en uso.", "MENSAJE EAGLE", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                errorProv.SetError(txtCodigo, "El código ya está en uso.");
            ...
            if (no_error && !chkActivo.Checked && TipoOperacion == TipoOperacionABM.Modificar)
            {
                ...
                if (int.TryParse(lblIdTipoProd.Text, out id))
                {
                    ...
                    validDelete = new UtilBL().ValidarDelete(id, CodValDelete.TipoProd_Producto);
                    if (!validDelete)
                    {
        ...
        private void Filtrar(int criterio, string filtro)
        {
            ...
            try
            {
                ...
                {
                    DataGridViewRow row = dgvTipoProd.Rows
                    ...
                    {
                        index = row.Index;
                        if (dgvTipoProd.Rows.Count > 0)
                        {
                            dgvTipoProd.Rows[index].Selected = true;
                            dgvTipoProd.FirstDisplayedScrollingRowIndex = index;
        ...
        private void SeleccionarPorId(int id)
        {
            ...
            try
            {
                ...
                DataGridViewRow row = dgvTipoProd.Rows
                .Cast<DataGridViewRow>()
                .Where(r => r.Cells["id_tipo_prod"].Value.ToString().Equals(id.ToString()))
                .FirstOrDefault();
                if (row != null)
                {
                    index = row.Index;
        ...
        {
            isPending = false;
            if (dgvTipoProd.RowCount > 0 && dgvTipoProd.SelectedRows.Count > 0 && dgvTipoProd.CurrentRow.Index != -1)
            {
                int id = 0;
                if (int.TryParse(GetIdSelected(), out id))
                {
                    if (id > 0)
                    {
                        var obj = new TipoProdBL().TipoProdXId(id);
                        if (obj != null)
                        {
        ...
        private string GetIdSelected()
        {
            ...
            try
            {
                if (dgvTipoProd.SelectedRows.Count > 0 && dgvTipoProd.Rows.Count > 0)
                {
                    id = dgvTipoProd.SelectedRows[0].Cells[0].Value.ToString();
                }
            }
            catch (Exception e)
            {
        ...
        private void CargarGrilla(int? id_estado = null)
        {
            try
            {
                var lista = new TipoProdBL().ListaTipoProd(id_estado, true);
                var listaView = lista.Select(x => new { x.id_tipo_prod, CODIGO = x.cod_tipo_prod, NOMBRE = x.txt_desc })
                .OrderBy(x => string.IsNullOrEmpty(x.CODIGO)).ThenBy(x => x.CODIGO, new AlphaNumericComparer()).ThenBy(x => x.NOMBRE).ToList();

                if (lista != null)
                {
                    ContarEstados(lista);
                    dgvTipoProd.DataSource = listaView;
                    dgvTipoProd.Columns["id_tipo_prod"].Visible = false;
                }
            }
            catch (Exception e)
            {
        ...
        {
            dgvTipoProd.RowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#ecf0f1");
            dgvTipoProd.AlternatingRowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#FAFAFA");

            //Cabecera
            dgvTipoProd.ColumnHeadersDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#00B2EE");
            dgvTipoProd.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            //Selección
            dgvTipoProd.DefaultCellStyle.SelectionBackColor = Color.DeepSkyBlue;

            //Para que no sobreescriba los estilos de cabecera
            dgvTipoProd.EnableHeadersVisualStyles = false;

            //Configurando columnas del grid
            dgvTipoProd.AllowUserToResizeColumns = true;
            dgvTipoProd.Columns["CODIGO"].HeaderText = "CÓDIGO";

            dgvTipoProd.Columns["CODIGO"].Width = 100;
            dgvTipoProd.Columns["NOMBRE"].Width = 300;
        }
        ...
        private void ContarEstados(List<PROt07_tipo_prod> lista)
        {
            try
            {
        ...
        private void dgvTipoProd_SelectionChanged(object sender, EventArgs e)
        {
            errorProv.Clear();
            if (isPending)
            {
                if (preguntar)
                {
                    ...
                    if (rp == DialogResult.Yes)
                    {
                        ...
                        if (idSelect != lblIdTipoProd.Text && idSelect != "-1")
                        {
                            isValid = Actualizar();
                            if (isValid)
                            {
