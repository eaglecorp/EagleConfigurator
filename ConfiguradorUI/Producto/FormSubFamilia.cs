...
{
    public partial class FormSubFamilia : MetroForm
    {
        #region Variables
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Insertar)
                {
                    if (esValido())
                    {
                        ...
                        obj = GetObjeto();
                        int id = new SubFamiliaBL().InsertarSubFam(obj);
                        if (id > 0)
                            actualizar = true;
        ...
        private void Eliminar()
        {
            ...
            {
                if (dgvSubfamilia.RowCount > 0)
                {
                    if (dgvSubfamilia.SelectedRows.Count > 0)
                    {
                        try
                        {
                            int id = 0;
                            if (int.TryParse(lblIdSubFamilia.Text, out id) && id > 0)
                            {
                                ...
                                {
                                    bool validDelete = new UtilBL().ValidarDelete(id, CodValDelete.SubFamilia_Producto);
                                    if (validDelete)
                                    {
                                        new SubFamiliaBL().EliminarSubFam(id);
                                        actualizar = true;
                                        ControlarEventosABM();
                                    }
                                    else
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Modificar && isSelected && isPending)
                {
                    if (esValido())
                    {
                        ...
                        obj = GetObjeto();
                        int id = 0;
                        if (int.TryParse(lblIdSubFamilia.Text, out id))
                        {
                            obj.id_subfamilia = id;
                            new SubFamiliaBL().ActualizarSubFam(obj);
                            actualizar = true;
                            ControlarEventosABM(obj.id_subfamilia);
                        }
                        isValid = true;
                    }
                    else { isValid = false; }
        ...
        private PROt04_subfamilia GetObjeto()
        {
            var obj = new PROt04_subfamilia();
            try
            {
                obj.txt_desc = txtNombre.Text.Trim();
                obj.cod_subfamilia = txtCodigo.Text.Trim();
        ...
        private void SetObjeto(PROt04_subfamilia obj)
        {
            try
            {
                ...
                lblIdSubFamilia.Text = obj.id_subfamilia.ToString();
                codSelected = obj.cod_subfamilia;

                txtNombre.Text = obj.txt_desc;
                txtCodigo.Text = obj.cod_subfamilia;
        ...
        private bool esValido()
        {
            ...
            {
                tabSubFamilia.SelectedTab = tabPagGeneral;
            ...
            if (no_error)
            {
                ...
                if (cod.Length > 0)
                {
                    var obj = new SubFamiliaBL().SubFamiliaXCod(cod);
                    if (TipoOperacion == TipoOperacionABM.Insertar)
                    {
                        if (obj != null && obj.id_subfamilia > 0)
                        {
                            tabSubFamilia.SelectedTab = tabPagGeneral;
                    ...
                    {
                        if (cod != codSelected && obj != null && obj.id_subfamilia > 0)
                        {
                            tabSubFamilia.SelectedTab = tabPagGeneral;
                            MessageBox.Show("El código ya está en uso.", "MENSAJE EAGLE", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            errorProv.SetError(txtCodigo, "El código ya está en uso.");
            ...
            if (no_error && !chkActivo.Checked && TipoOperacion == TipoOperacionABM.Modificar)
            {
                ...
                if (int.TryParse(lblIdSubFamilia.Text, out id))
                {
                    ...
                    validDelete = new UtilBL().ValidarDelete(id, CodValDelete.SubFamilia_Producto);
                    if (!validDelete)
                    {
        ...
        private void Filtrar(int criterio, string filtro)
        {
            ...
            try
            {
                ...
                {
                    DataGridViewRow row = dgvSubfamilia.Rows
                    ...
                    {
                        index = row.Index;
                        if (dgvSubfamilia.Rows.Count > 0)
                        {
                            dgvSubfamilia.Rows[index].Selected = true;
                            dgvSubfamilia.FirstDisplayedScrollingRowIndex = index;
        ...
        private void SeleccionarPorId(int id)
        {
            ...
            try
            {
                ...
                DataGridViewRow row = dgvSubfamilia.Rows
                .Cast<DataGridViewRow>()
                .Where(r => r.Cells["id_subfamilia"].Value.ToString().Equals(id.ToString()))
                .FirstOrDefault();
                if (row != null)
                {
                    index = row.Index;
        ...
        {
            isPending = false;
            if (dgvSubfamilia.RowCount > 0 && dgvSubfamilia.SelectedRows.Count > 0 && dgvSubfamilia.CurrentRow.Index != -1)
            {
                int id = 0;
                if (int.TryParse(GetIdSelected(), out id))
                {
                    if (id > 0)
                    {
                        var obj = new SubFamiliaBL().SubFamiliaXId(id);
                        if (obj != null)
                        {
                            isSelected = false;
                            SetObjeto(obj);
        ...
        private string GetIdSelected()
        {
            ...
            try
            {
                if (dgvSubfamilia.SelectedRows.Count > 0 && dgvSubfamilia.Rows.Count > 0)
                {
                    id = dgvSubfamilia.SelectedRows[0].Cells[0].Value.ToString();
                }
            }
            catch (Exception e)
            {
        ...
        private void CargarGrilla(int? id_estado = null)
        {
            try
            {
               ...
               .OrderBy(x => string.IsNullOrEmpty(x.CODIGO)).ThenBy(x => x.CODIGO, new AlphaNumericComparer()).ThenBy(x => x.NOMBRE).ToList();
                ...
                {
                    ContarEstados(lista);
                    dgvSubfamilia.DataSource = listaView;
                    dgvSubfamilia.Columns["id_subfamilia"].Visible = false;
                }
            }
            catch (Exception e)
            {
        ...
        {
            dgvSubfamilia.RowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#ecf0f1");
            dgvSubfamilia.AlternatingRowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#FAFAFA");

            //Cabecera
            dgvSubfamilia.ColumnHeadersDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#00B2EE");
            dgvSubfamilia.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            //Selección
            dgvSubfamilia.DefaultCellStyle.SelectionBackColor = Color.DeepSkyBlue;
        ...
        private void ContarEstados(List<PROt04_subfamilia> lista)
        {
            try
            {
        ...
        private void dgvSubfamilia_SelectionChanged(object sender, EventArgs e)
        {
            errorProv.Clear();
            if (isPending)
            {
                if (preguntar)
                {
                    ...
                    if (rp == DialogResult.Yes)
                    {
                        ...
                        if (idSelect != lblIdSubFamilia.Text && idSelect != "-1")
                        {
                            isValid = Actualizar();
                            if (isValid)
                            {
        ...
        #region Evento de ventanas emergentes
