...
namespace ConfiguradorUI.Producto
{
    ...
    public partial class FormMarca : MetroForm
    {
        #region Variables
        ...
        private void Commit()
        {
            try
            {
                if (TipoOperacion == TipoOperacionABM.Insertar)
                {
                    if (esValido())
                    {
                        PROt01_marca obj = new PROt01_marca();
                        obj = GetObjeto();
                        int id = new MarcaBL().InsertarMarca(obj);
                        if (id > 0)
                            actualizar = true;
            ...
            if (TipoOperacion == TipoOperacionABM.Eliminar)
            {
                if (dgvMarca.RowCount > 0)
                {
                    if (dgvMarca.SelectedRows.Count > 0)
                    {
                        ...
                        {
                            int id = 0;
                            if (int.TryParse(lblIdMarca.Text, out id) && id > 0)
                            {
                                ...
                                {
                                    bool validDelete = new UtilBL().ValidarDelete(id, CodValDelete.Marca_Modelo);
                                    if (validDelete)
                                    {
                                        new MarcaBL().EliminarMarca(id);
                                        actualizar = true;
                                        ControlarEventosABM();
                                    }
                                    else
            ...
            try
            {
                if (TipoOperacion == TipoOperacionABM.Modificar && isSelected && isPending)
                {
                    if (esValido())
                    {
                        ...
                        obj = GetObjeto();
                        int id = 0;
                        if (int.TryParse(lblIdMarca.Text, out id))
                        {
                            obj.id_marca = id;
                            new MarcaBL().ActualizarMarca(obj);
                            actualizar = true;
                            ControlarEventosABM(obj.id_marca);
                        }
                        isValid = true;
                    }
                    else { isValid = false; }
        ...
        private PROt01_marca GetObjeto()
        {
            PROt01_marca obj = new PROt01_marca();
            try
            {
                obj.txt_desc = txtNombre.Text.Trim();
                obj.cod_marca = txtCodigo.Text.Trim();
        ...
        private void SetObjeto(PROt01_marca obj)
        {
            try
            {
                ...
                lblIdMarca.Text = obj.id_marca.ToString();
                codSelected = obj.cod_marca;

                txtNombre.Text = obj.txt_desc;
                txtCodigo.Text = obj.cod_marca;
        ...
        private bool esValido()
        {
            ...
            {
                tabMarca.SelectedTab = tabPagGeneral;
            ...
            if (no_error)
            {
                ...
                if (cod.Length > 0)
                {
                    ...
                    {
                        var obj = new MarcaBL().MarcaXCod(cod);
                        if (TipoOperacion == TipoOperacionABM.Insertar)
                        {
                            if (obj != null && obj.id_marca > 0)
                            {
                                tabMarca.SelectedTab = tabPagGeneral;
                        ...
                        {
                            if (cod != codSelected && obj != null && obj.id_marca > 0)
                            {
                                tabMarca.SelectedTab = tabPagGeneral;
                                MessageBox.Show("El código ya está en uso.", "MENSAJE EAGLE", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                errorProv.SetError(txtCodigo, "El código ya está en uso.");
            ...
            if (no_error && !chkActivo.Checked && TipoOperacion == TipoOperacionABM.Modificar)
            {
                ...
                if (int.TryParse(lblIdMarca.Text, out id))
                {
                    ...
                    validDelete = new UtilBL().ValidarDelete(id, CodValDelete.Marca_Modelo);
                    if (!validDelete)
                    {
        ...
        private void Filtrar(int criterio, string filtro)
        {
            ...
            try
            {
                ...
                {
                    DataGridViewRow row = dgvMarca.Rows
                    ...
                    if (row != null)
                    {
                        ...
                        {
                            dgvMarca.Rows[index].Selected = true;
                            dgvMarca.FirstDisplayedScrollingRowIndex = index;
                        }
                    }
                }
        ...
        private void SeleccionarPorId(int id)
        {
            ...
            try
            {
                ...
                DataGridViewRow row = dgvMarca.Rows
                .Cast<DataGridViewRow>()
                .Where(r => r.Cells["id_marca"].Value.ToString().Equals(id.ToString()))
                .FirstOrDefault();
                if (row != null)
                {
                    index = row.Index;
        ...
        {
            isPending = false;
            if (dgvMarca.RowCount > 0 && dgvMarca.SelectedRows.Count > 0 && dgvMarca.CurrentRow.Index != -1)
            {
                int id = 0;
                if (int.TryParse(GetIdSelected(), out id))
                {
                    if (id > 0)
                    {
                        var obj = new MarcaBL().MarcaXId(id);
                        if (obj != null)
                        {
        ...
        private string GetIdSelected()
        {
            ...
            try
            {
                if (dgvMarca.SelectedRows.Count > 0 && dgvMarca.Rows.Count > 0)
                {
                    id = dgvMarca.SelectedRows[0].Cells[0].Value.ToString();
                }
            }
            catch (Exception e)
            {
        ...
        private void CargarGrilla(int? id_estado = null)
        {
            try
            {
                var lista = new MarcaBL().ListaMarca(id_estado, true);
                var listaView = lista.Select(x => new { x.id_marca, CODIGO = x.cod_marca, NOMBRE = x.txt_desc })
                .OrderBy(x => string.IsNullOrEmpty(x.CODIGO)).ThenBy(x => x.CODIGO, new AlphaNumericComparer()).ThenBy(x => x.NOMBRE).ToList();

                if (lista != null)
                {
                    ContarEstados(lista);
                    dgvMarca.DataSource = listaView;
        ...
        {
            dgvMarca.RowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#ecf0f1");
            dgvMarca.AlternatingRowsDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#FAFAFA");

            //Cabecera
            dgvMarca.ColumnHeadersDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#00B2EE");
            dgvMarca.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            //Selección
            dgvMarca.DefaultCellStyle.SelectionBackColor = Color.DeepSkyBlue;

            //Para que no sobreescriba los estilos de cabecera
            dgvMarca.EnableHeadersVisualStyles = false;

            //Configurando columnas del grid
            dgvMarca.AllowUserToResizeColumns = true;
            dgvMarca.Columns["CODIGO"].HeaderText = "CÓDIGO";

            dgvMarca.Columns["CODIGO"].Width = 100;
            dgvMarca.Columns["NOMBRE"].Width = 300;
        }
        ...
        private void ContarEstados(List<PROt01_marca> lista)
        {
            try
            {
            ...
            if (isPending)
            {
                if (preguntar)
                {
                    ...
                    if (rp == DialogResult.Yes)
                    {
                        ...
                        if (idSelect != lblIdMarca.Text && idSelect != "-1")
                        {
                            isValid = Actualizar();
                            if (isValid)
                            {
