...
namespace ConfigBusinessLogic.Persona
{
    public class ClaseEmpBL
    {
        public List<PERt06_clase_emp> ListaClaseEmp(int? id_estado = null, bool ocultarBlankReg = false, bool enableTopList = false)
        {
            ...
            {
                var itemToRemove = lista.SingleOrDefault(x => x.cod_clase_emp == Parameter.BlankRegister);
                if (itemToRemove != null && itemToRemove.id_clase_emp > 0)
                    lista.Remove(itemToRemove);
            }

            if (enableTopList && lista != null)
                return lista.OrderBy(x => x.cod_clase_emp != TopList.ClaseEmp).ThenBy(x => x.txt_desc).ToList();
